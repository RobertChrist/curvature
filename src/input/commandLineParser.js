...
var args = [
	...
	[ ['--' + settings.limitPoints.name],		{ action: 'int', 		defaultValue: settings.limitPoints.value,		help: 'The maximum number of points to used to render each line, 0 for all points. The default is 0. Must be 0 or greater than or equal to 2.  Only available to single color kml files.'}],
	[ ['--' + settings.relativeColor.name],		{ action: 'bool', 		defaultValue: settings.relativeColor.value,		help: 'Make the color-scale relative to the maximum curvature in the input file when using Single Color output.'}],
